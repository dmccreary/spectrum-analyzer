# Using an Assembler Version of the FFT

![FFT 2K Demo](fft-2k-demo.png)

In many signal processing applications, transforming time
series data into the frequency domain is a CPU intensive operation.
For many systems, having a very fast FFT in assembler is critical.

In October 2015, Peter Hinch posted a version of the FFT algorithm on [his GitHub Repo](https://github.com/peterhinch/micropython-fourier/blob/master/dft.py) written in ARM assembly.  We can use this
function to increase the speed of our FFT by a factor of 7x or more.  This `dft.py` program should be uploaded to the `/lib` directory and then call it like this:
...
From Peter's README.me:

    Raspberry Pi Pico 2 uses the dual-core ARM Cortex-M33 clocked at 150MHz CPU. ARM Thumb V7 instruction set with an FPU (Floating Point Unit).  The ARM FPU is so fast that integer code offers no speed advantage.

## Pico 2 RP2350 Signal Processing Hardware

The ARM Cortex-M33 on the Pico 2 is very different from the original Pico.  It has faster single precision floating point and specialized DSP extensions specifically added to increase the performance of signal processing.  Specifically the Pico 2 has:

- **CPU** ARM Cortex-M33, dual-core — ideal for real-time + background task separation
- **Instruction Set:** ARMv8-M + Thumb-2
- **FPU:** Single-precision floating point unit (FPv5-SP)
- **DSP Extensions:** Yes (for signal processing tasks)

Dual-core with independent execution could allow the FFT to run continiously on a second core while the first core gathers sound data and displays the results.

## Full Main Code for Calling Assembler

[Source Code](https://github.com/dmccreary/learning-micropython/blob/main/src/kits/fft/56-fft-asm.py)

Here is sample `main.py` code for running your spectrum analyzer.
